    ...
    switch (paymentGatewaysEnum) {
      ...
      case PaymentGatewaysEnum.macroclick:
        ...
        if (request.url.contains("ReciboPago?token=")) {
          result["status"] = "approved";
          String link = request.url.substring(0, lastURL.indexOf("&"));
          result["link"] = link;
          String downloadLink = link;
          downloadLink.replaceAll("ReciboPago", "ReciboPagoPdf");
          result["downloadLink"] = downloadLink;
          onBack(context);
          return NavigationDecision.prevent;
        }

        if (request.url.contains("ReciboPagoPdf?token=")) {
          result["status"] = "approved";
          result["downloadLink"] = request.url;
          String proofOfPaymentLink = request.url;
          proofOfPaymentLink.replaceAll("ReciboPagoPdf", "ReciboPago");
          result["link"] = proofOfPaymentLink;
